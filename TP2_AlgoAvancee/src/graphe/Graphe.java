...
public class Graphe {
	...
	public void ufUnion(Sommet sommet1Head, Sommet sommet2Head, Arete arete){
		if(sommet1Head.getHeight() >= sommet2Head.getHeight()) {
			//arete.sommet2.setHead(sommet1Head);
			...
			//arete.sommet1.setHead(arete.sommet2);
			for(Sommet sommetIndex : listeSommet)
			{
				if(sommetIndex.getHead().getNumero() == sommet2Head.getNumero())
					sommetIndex.setHead(sommet1Head);
			}
			//arete.sommet2.setHeight(arete.sommet2.getHeight() + 1);
		}
	}
	public ArrayList<Arete> algoPrime(int nomSommet) {
		Sommet sommetDepart = chercheSommet(nomSommet);
		if (sommetDepart == null){
			System.out.println("le sommet de depart indiqué n'existe pas !");
			return null;
		}
	...
	public ArrayList<Arete> algoPrimeTas(int nomSommet) {
		...
		//ArrayList<Arete> aretesConsultables = new ArrayList();
		ArrayList<Arete> aretesResultat = new ArrayList();
		ArrayList<Sommet> sommetResultat = new ArrayList();
		Arete petiteAret;
		sommetResultat.add(sommetDepart);
	...
	public ArrayList<Arete> algoKruskal() {
		ArrayList<Arete> aretes = this.listeArete;
		ArrayList<Arete> aretesResultat = new ArrayList();
		ArrayList<Sommet> sommetResultat = new ArrayList();
		Arete petiteAret;	
		while(aretesResultat.size() != this.listeSommet.size() - 1){
			...
			/*if(!(isCycle(petiteAret.sommet2, petiteAret.sommet1, aretesResultat))) {
				sommetResultat.add(petiteAret.sommet2);
				if (!aretesResultat.contains(petiteAret))
					aretesResultat.add(petiteAret);
			}*/
			aretes.remove(petiteAret);
		}
	...
	/*public boolean isCycle(Sommet sommet1, Sommet sommet2, ArrayList<Arete> listeArete){
		if(sommet1 == sommet2)
			return true;
		//ArrayList<Arete> currentArete = new ArrayList();
		for (int index = 0; index < listeArete.size(); index++){
			if (listeArete.get(index).sommet1 == sommet1)
				return isCycle(listeArete.get(index).sommet2, sommet2, listeArete);
			else if(listeArete.get(index).sommet2 == sommet1)
				return isCycle(listeArete.get(index).sommet1, sommet2, listeArete);
		}
		//for(int index = 0; index < currenArete.)
		return false;
	}*/
	
	public boolean isCycle(Sommet sommet1, Sommet sommetRecherche, ArrayList<Arete> listeArete){ //à la premiere iteration, sommet1 est un des deux sommets de l'arete, et sommetRecherche est l'autre sommet de cette arete
